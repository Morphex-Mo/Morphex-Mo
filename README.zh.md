<h1 align="center">
  
[![Typing SVG](https://readme-typing-svg.herokuapp.com?color=bf91f3&size=30&center=true&vCenter=true&width=500&height=35&lines=Welcome!;+I'm+M0rphex+莫晓喵!)](https://git.io/typing-svg)

</h1>

[English](README.md) | [中文](README.zh.md) | [日本語](README.jp.md)

# 技术爱好者 & 计算机科学学生 👨‍💻

## 🎓 教育背景  
**计算机科学工程学士**  
南方科技大学 (2023-2025)  
香港理工大学 (2025-2026)  
南方科技大学 (2026-至今)  
相关课程：Java 编程、机器学习（自学）  
*"将好奇心转化为代码"* ✨  

## 🔍 项目与研究  
**机器学习研究** @ 大学实验室 (2025年至今)  
▸ 自学注意力机制与CNN架构 👨‍🔬  
▸ 准备加入教授研究团队  

**Java身份识别扫描器GUI** @ 技术实习 (2024年8月)  
▸ 开发了数据可视化界面 🖥️  
▸ 因高效实现获得认可  

**智能创新竞赛** (2024年5月)  
▸ 领导原型开发团队获得第二名 🏆  
▸ 在竞争激烈的环境中设计创新解决方案  

**Java益智游戏** (2023年11月)  
▸ 合作开发配对游戏入选课堂展示 🎮  

**RISC-V CPU设计** (2024-2025年)  
▸ 使用Verilog和RISC-V指令集从零设计并实现功能完整的CPU  
▸ 成功执行基础算术与逻辑运算  

**跨文化翻译AI** (2025年)  
▸ 开发AI翻译工具，提供语境感知翻译与文化适应建议  
▸ 集成先进NLP技术处理复杂语言场景  

**网络化区块链模拟系统** (2025年)  
▸ 基于Docker平台使用套接字编程构建安全的模拟区块链交易系统  
▸ 实现了计算机网络课程中的核心网络与加密概念  

**弹幕游戏开发** (2025年)  
▸ 与同学合作设计开发弹幕射击游戏  
▸ 因创意设计与流畅玩法获得教师好评  

**抽油烟机控制系统模拟** (2024年)  
▸ 团队合作使用Verilog模拟全功能抽油烟机控制系统  
▸ 完美实现所有要求功能  

## 🌟 领导力与活动  
**大学招生推广项目** (2024年1月)  
▸ 向潜在新生介绍校园生活 🎤  
▸ 因团队协作与问答环节设计获奖  

## ⚙️ 技术工具包  
**编程语言:** Java (精通), Python, C++, Verilog, RISC-V汇编  
**开发工具:** Git, Docker, 套接字编程  
**技术概念:** 机器学习 • CNN • 注意力机制 • 计算机体系结构 • 区块链 • 游戏开发 • 自然语言处理  
**语言能力:** 英语 (流利) • 日语 (学习中) • 中文 (母语)  

## 🏆 获奖荣誉  
• 智能创新竞赛 - 第二名  
• 招生推广项目 - 优秀个人与团队奖  
• 课程项目 - 顶级课堂展示入选  
• 数字设计课程 - 抽油烟机系统完美实现奖  
• 计算机体系结构项目 - CPU设计优秀奖  
• 软件工程课程 - 游戏开发获得教师表彰  

## ⚽️ 键盘之外的生活  
当不编码时，你会发现我：  
▸ 在篮球场上运球上篮 🏀  
▸ 在羽毛球场上扣杀 🏸  
▸ 在游泳池中游 laps 🏊‍♂️  
▸ 在深夜足球比赛中欢呼 ⚽️  
▸ 探索计算机音频技术 🎧  

*"平衡算法精度与运动活力 – 逻辑与激情的交汇"* 🌈  

---

### 让我们连接并构建有影响力的东西！ 🤝  

<div align="center">
  <img src="https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExdmpmNXhmbG93ZDB1Mms0eGFqMHZpZXhsZzV0d2lycDI4d3I3Y3ZsdyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/11lxCeKo6cHkJy/giphy.gif" alt="明日香跳舞" width="300">
  
  <br><br>
  
  <div style="display: flex; justify-content: center; gap: 20px;">
    <img src="https://github-readme-stats-ten-dusky-26.vercel.app/api?username=TonyMo0310&theme=vue-dark&show_icons=true&hide_border=true&count_private=true" alt="GitHub Stats" width="400">
  </div>
  
  <br>
  
  ![GitHub Contribution Snake](https://raw.githubusercontent.com/TonyMo0310/TonyMo0310/output/snake.svg?palette=github-light#gh-light-mode-only)
  
  <br>
  
  <img src="https://raw.githubusercontent.com/TonyMo0310/TonyMo0310/output/snake.gif" alt="Snake Animation" width="100%">
  
  <br><br>
  
  <img src="https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExd2lvdGh0bG9qZ3VnZzlmdnY2djduaXBmaGl1cm1tMWluMHRmcjczYyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/a6pzK009rlCak/giphy.gif" alt="魔王邪眼小鸟游六花" width="300">
</div>
