<h1 align="center">
  
[![Typing SVG](https://readme-typing-svg.herokuapp.com?color=bf91f3&size=30&center=true&vCenter=true&width=500&height=35&lines=Welcome!;+I'm+M0rphex+莫晓喵!)](https://git.io/typing-svg)

</h1>

## 🌐 Multi-language | 多语言 | 多言語

- [English](#english)
- [中文](#中文)
- [日本語](#日本語)

---

## <a id="english"></a>English

### Tech Enthusiast & Computer Science Student 👨‍💻

#### 🎓 Education
**Bachelor of Engineering in Computer Science**  
Southern University of Science and Technology (2023-2025)  
The Hong Kong Polytechnic University (2025-2026)  
Southern University of Science and Technology (2026-Present)  
Relevant Coursework: Java Programming, Machine Learning (self-directed)  
*"Transforming curiosity into code"* ✨  

#### 🔍 Projects & Research
- **Machine Learning Research** @ University Lab (2025-Present)
  - Self-studying attention mechanisms and CNN architectures 👨‍🔬
  - Preparing to join faculty research group
- **Java ID Scanner GUI** @ Tech Internship (08/2024)
  - Developed data visualization interface 🖥️
  - Recognized for efficient implementation
- **Intelligent Innovation Competition** (05/2024)
  - Led prototype development team to 2nd prize finish 🏆
  - Designed innovative solution in competitive pool
- **Java Puzzle Game** (11/2023)
  - Co-created matching game selected for class showcase 🎮
- **RISC-V CPU Design** (2024-2025)
  - Designed and implemented a functional CPU from scratch using Verilog and RISC-V ISA
  - Successfully executed basic arithmetic and logic operations
- **Cross-Cultural Translation AI** (2025)
  - Developed AI-powered translation tool with context-aware/cultural adaptation
  - Integrated advanced NLP for nuanced language
- **Networked Blockchain Simulation** (2025)
  - Built a secure, simulated blockchain transaction system using socket programming on Docker
  - Implemented core networking & cryptographic concepts
- **Danmaku-Style Game** (2025)
  - Designed & developed a bullet-hell game, received positive faculty feedback
- **Range Hood Control System Simulation** (2024)
  - Simulated a full-featured control system using Verilog. Perfect functionality

#### 🌟 Leadership & Activities
- University Outreach Program (01/2024)
  - Presented campus life sessions 🎤
  - Awarded for team coordination/Q&A design

#### ⚙️ Technical Toolkit
**Languages:** Java (Proficient), Python, C++, Verilog, RISC-V Assembly  
**Tools:** Git, Docker, Socket Programming  
**Concepts:** Machine Learning • CNNs • Attention Mechanisms • Architecture • Blockchain • Game Dev • NLP  
**Languages:** English (Fluent) • Japanese (Learning) • Mandarin (Native)

#### 🏆 Recognitions
- Intelligent Innovation Competition - 2nd Prize  
- Outreach - Outstanding Individual & Team  
- Course Project - Top-Tier Showcase  
- Digital Design - Perfect Implementation  
- Architecture - Excellence in CPU  
- Software Engineering - Commendation for Game Dev

#### ⚽️ Beyond the Keyboard  
Basketball 🏀, Badminton 🏸, Swimming 🏊‍♂️, Late-night football ⚽️, Audio tech 🎧

*"Balancing algorithmic precision with athletic energy — logic meets passion"* 🌈


## <a id="中文"></a>中文

### 技术爱好者 & 计算机科学学生 👨‍💻

#### 🎓 教育背景  
**��算机科学工程学士**  
南方科技大学 (2023-2025)  
香港理工大学 (2025-2026)  
南方科技大学 (2026-至今)  
相关课程：Java 编程、机器学习（自学）  
*"将好奇心转化为代码"* ✨  

#### 🔍 项目与研究  
- **机器学习研究** @ 大学实验室 (2025年-至今)  
  - 自学注意力机制与CNN架构 👨‍🔬  
  - 准备加入教授研究团队  
- **Java身份识别扫描器GUI** @ 技术实习 (2024年8月)  
  - 开发数据可视化界面 🖥️  
  - 因高效实现获得认可  
- **智能创新竞赛** (2024年5月)  
  - 领导原型开发团队获得第二名 🏆  
  - 设计创新解决方案  
- **Java益智游戏** (2023年11月)  
  - 合作开发配对游戏，入选课堂展示 🎮  
- **RISC-V CPU设计** (2024-2025年)  
  - 使用Verilog和RISC-V指令集从零实现CPU  
  - 成功执行算术与逻辑操作  
- **跨文化翻译AI** (2025年)  
  - 开发AI翻译工具，提供语境感知及文化建议  
  - 集成先进NLP技术  
- **区块链模拟系统** (2025年)  
  - 基于Docker使用套接字编程构建安全模拟系统  
  - 实现课程核心网络与加密概念  
- **弹幕游戏开发** (2025年)  
  - 合作开发弹幕游戏  
  - 创意和流畅度获教师好评  
- **抽油烟机控制系统模拟** (2024年)  
  - 团队使用Verilog模拟全功能系统  
  - 完美实现全部操作功能  

#### 🌟 领导力与活动  
- 大学招生推广项目 (2024年1月)  
  - 介绍校园生活 🎤  
  - 因团队协作与问答设计获奖  

#### ⚙️ 技术工具包  
**编程语言：** Java (精通), Python, C++, Verilog, RISC-V汇编  
**开发工具：** Git, Docker, 套接字编程  
**技术概念：** 机器学习 • CNN • 注意力机制 • 计算机体系结构 • 区块链 • 游戏开发 • NLP  
**语言能力：** 英语 (流利) • 日语 (学习中) • 中文 (母语)

#### 🏆 获奖荣誉  
- 智能创新竞赛 - 第二名  
- 招生推广项目 - 优秀个人与团队奖  
- 课程项目 - 顶级课堂展示  
- 数字设计 - 完美实现奖  
- 计算机体系结构项目 - 优秀CPU设计  
- 软件工程课程 - 教师表彰

#### ⚽️ 键盘之外  
篮球 🏀，羽毛球 🏸，游泳 🏊‍♂️，深夜足球 ⚽️，音频技术 🎧

*"平衡算法精度与运动活力 — 逻辑与激情的交汇"* 🌈


## <a id="日本語"></a>日本語

### 技術愛好者 & コンピュータサイエンス学生 👨‍💻

#### 🎓 教育
**コンピュータサイエンス工学学士**  
南方科技大学 (2023-2025)  
香港理工大学 (2025-2026)  
南方科技大学 (2026-現在)  
関連コース：Java プログラミング、機械学習（自己学習）  
*"好奇心をコードに変える"* ✨  

#### 🔍 プロジェクト & 研究
- **機械学習研究** @ 大学ラボ (2025-現在)  
  - アテンション機構とCNNアーキテクチャを自己学習 👨‍🔬  
  - 教員研究グループへの参加準備  
- **Java ID スキャナー GUI** @ 技術インターンシップ (2024年8月)  
  - データビジュアライゼーションインターフェースを開発 🖥️  
  - 効率的な実装で表彰  
- **インテリジェントイノベーションコンペティション** (2024年5月)  
  - 開発チームをリードし、2位入賞 🏆  
  - 革新的なソリューションを設計  
- **Java パズルゲーム** (2023年11月)  
  - 共同作成したマッチングゲームがクラスショーケースに選出 🎮  
- **RISC-V CPU 設計** (2024-2025)  
  - VerilogとRISC-V ISAでCPUを設計・実装  
  - 基本的な算術・論理演算を実行  
- **異文化対応翻訳 AI** (2025)
  - 文脈を考慮した翻訳と文化適応アドバイスを提供
  - 高度なNLP技術を統合
- **ブロックチェーンシミュレーション** (2025)
  - Dockerプラットフォームで模擬ブロックチェーン取引システムを構築
  - コアネットワーク・暗号概念を実装
- **弾幕ゲーム��発** (2025)
  - 弾幕シューティングゲームを設計・開発  
  - 教員から好評
- **レンジフード制御システムシミュレーション** (2024)
  - Verilogで全機能を備えたシステムをシミュレーション  
  - 全操作機能を実装

#### 🌟 リーダーシップ & 活動
- 大学アウトリーチプログラム (2024年1月)
  - キャンパスライフを紹介 🎤
  - チーム調整/Q&A設計で表彰

#### ⚙️ 技術ツールキット
**言語**: Java (熟練), Python, C++, Verilog, RISC-Vアセンブリ
**ツール**: Git, Docker, ソケットプログラミング
**概念**: 機械学習 • CNN • アテンション • アーキテクチャ • ブロックチェーン • ゲーム開発 • NLP
**言語能力**: 英語 (流暢) • 日本語 (学習中) • 中国語 (母語)

#### 🏆 表彰
- イノベーションコンペ - 2位
- アウトリーチ - 優秀個人＆チーム
- コースプロジェクト - トップショーケース
- デジタル設計 - 完全実装賞
- アーキテクチャ - 優秀CPU設計
- ソフトウェア工学 - 教員表彰

#### ⚽️ キーボードを超えて
バスケットボール 🏀、バドミントン 🏸、水泳 🏊‍♂️、深夜サッカー ⚽️、オーディオ技術 🎧

*"アルゴリズムの精度と運動エネルギーのバランス — 論理と情熱の出会い"* 🌈

---

<!--START_SECTION:waka-->

```txt
From: 09 February 2026 - To: 16 February 2026

Total Time: 2 hrs 34 mins

Java         1 hr 17 mins    🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🟠🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵   46.84 %
JavaScript   1 hr 2 mins     🔴🔴🔴🔴🔴🔴🔴🔴🔴🟡🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵   37.80 %
Other        11 mins         🔴🟠🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵   06.70 %
Markdown     11 mins         🔴🟠🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵   06.67 %
CSS          1 min           🟢🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵   01.11 %
Python       0 secs          🟢🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵   00.50 %
JSON         0 secs          🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵   00.16 %
GLSL         0 secs          🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵   00.09 %
HTML         0 secs          🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵   00.07 %
TypeScript   0 secs          🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵🔵   00.06 %
```

<!--END_SECTION:waka-->

---

<div align="center">
  <img src="https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExdmpmNXhmbG93ZDB1Mms0eGFqMHZpZXhsZzV0d2lycDI4d3I3Y3ZsdyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/11lxCeKo6cHkJy/giphy.gif" alt="明日香跳舞" width="300">
  <br><br>
  <!-- 推荐只保留一次 Stats 卡片 -->
  <img src="https://github-readme-stats-three-zeta-95.vercel.app/api?username=Morphex-Mo&show_icons=true&theme=vue-dark" alt="GitHub Stats" width="400">
  <br>
  
  ![GitHub Contribution Snake](https://raw.githubusercontent.com/TonyMo0310/TonyMo0310/output/snake.svg#gh-dark-mode-only)
  <br><br>
  <img src="https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExd2lvdGh0bG9qZ3VnZzlmdnY2djduaXBmaGl1cm1tMWluMHRmcjczYyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/a6pzK009rlCak/giphy.gif" alt="魔王邪眼小鸟游六花" width="300">
</div>
