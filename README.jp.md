<h1 align="center">
  
[![Typing SVG](https://readme-typing-svg.herokuapp.com?color=bf91f3&size=30&center=true&vCenter=true&width=500&height=35&lines=Welcome!;+I'm+M0rphex+莫晓喵!)](https://git.io/typing-svg)

</h1>

[English](README.md) | [中文](README.zh.md) | [日本語](README.jp.md)

# 技術愛好者 & コンピュータサイエンス学生 👨‍💻

## 🎓 教育  
**コンピュータサイエンス工学学士**  
南方科技大学 (2023-2025)  
香港理工大学 (2025-2026)  
南方科技大学 (2026-現在)  
関連コースワーク：Java プログラミング、機械学習（自己学習）  
*"好奇心をコードに変える"* ✨  

## 🔍 プロジェクト & 研究  
**機械学習研究** @ 大学ラボ (2025-現在)  
▸ アテンション機構とCNNアーキテクチャを自己学習 👨‍🔬  
▸ 教員研究グループへの参加準備  

**Java ID スキャナー GUI** @ 技術インターンシップ (2024年8月)  
▸ データビジュアライゼーションインターフェースを開発 🖥️  
▸ 効率的な実装で表彰  

**インテリジェントイノベーションコンペティション** (2024年5月)  
▸ プロトタイプ開発チームをリードし、2位入賞 🏆  
▸ 競争の激しい環境で革新的なソリューションを設計  

**Java パズルゲーム** (2023年11月)  
▸ 共同作成したマッチングゲームがクラスショーケースに選出 🎮  

**RISC-V CPU 設計** (2024-2025)  
▸ VerilogとRISC-V ISAを使用してゼロから機能的なCPUを設計・実装  
▸ 基本的な算術・論理演算の実行に成功  

**異文化対応翻訳 AI** (2025)  
▸ 文脈を考慮した翻訳と文化的適応ア��バイスを提供するAI翻訳ツールを開発  
▸ 高度なNLP技術を統合し、微妙な言語シナリオに対応  

**ネットワーク型ブロックチェーンシミュレーション** (2025)  
▸ Dockerプラットフォーム上でソケットプログラミングを使用した安全な模擬ブロックチェーン取引システム構築  
▸ コンピュータネットワーク講座で学んだコアネットワーク・暗号概念を実装  

**弾幕ゲーム開発** (2025)  
▸ 同級生と協力し、弾幕シューティングゲームを設計・開発  
▸ 創造的なデザインと滑らかなゲームプレイで教員から好評  

**レンジフード制御システムシミュレーション** (2024)  
▸ チームでVerilogを使用し全機能を備えたレンジフード制御システムをシミュレーション  
▸ 全ての操作機能を完璧に実装  

## 🌟 リーダーシップ & 活動  
**大学アウトリーチプログラム** (2024年1月)  
▸ 潜在的な学生にキャンパスライフを紹介 🎤  
▸ チーム調整とQ&A設計で表彰  

## ⚙️ 技術ツールキット  
**プログラミング言語:** Java (熟練)、Python、C++、Verilog、RISC-Vアセンブリ  
**開発ツール:** Git、Docker、ソケットプログラミング  
**技術概念:** 機械学習 • CNN • アテンション機構 • コンピュータアーキテクチャ • ブロックチェーン • ゲーム開発 • NLP  
**言語能力:** 英語 (流暢) • 日本語 (学習中) • 中国語 (母語)  

## 🏆 表彰  
• インテリジェントイノベーションコンペティション - 2位  
• アウトリーチプログラム - 優秀個人＆チーム賞  
• コースプロジェクト - トップティアクラスショーケース選出  
• デジタルデザイン講座 - レンジフードシステム完全実装賞  
• コンピュータアーキテクチャプロジェクト - CPU設計優秀賞  
• ソフトウェアエンジニアリング講座 - ゲーム開発で教員表彰  

## ⚽️ キーボードを超えて  
コードを書いていない時は、私を見つけることができます：  
▸ バスケットボールコートでバスケットに向かってドライブ 🏀  
▸ バドミントンコートでバーディーをスマッシュ 🏸  
▸ プールでラップ泳ぎ 🏊‍♂️  
▸ 深夜のサッカーマッチで応援 ⚽️  
▸ コンピュータオーディオ技術を探求 🎧  

*"アルゴリズムの精度と運動エネルギーのバランス – 論理と情熱の出会い"* 🌈  

---

### つながって影響力のあるものを構築しましょう！ 🤝  

<div align="center">
  <img src="https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExdmpmNXhmbG93ZDB1Mms0eGFqMHZpZXhsZzV0d2lycDI4d3I3Y3ZsdyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/11lxCeKo6cHkJy/giphy.gif" alt="明日香跳舞" width="300">
  
  <br><br>
  
  <div style="display: flex; justify-content: center; gap: 20px;">
    <img src="https://github-readme-stats-ten-dusky-26.vercel.app/api?username=TonyMo0310&theme=vue-dark&show_icons=true&hide_border=true&count_private=true" alt="GitHub Stats" width="400">
  </div>
  
  <br>

<!--START_SECTION:waka-->
<!-- 以下は過去7日間のWakaTime統計です。Actionにより自動更新 -->
<!--END_SECTION:waka-->

  <br><br>
  
  ![GitHub Contribution Snake](https://raw.githubusercontent.com/TonyMo0310/TonyMo0310/output/snake.svg#gh-dark-mode-only)
  
  <br><br>
  
  <img src="https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExd2lvdGh0bG9qZ3VnZzlmdnY2djduaXBmaGl1cm1tMWluMHRmcjczYyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/a6pzK009rlCak/giphy.gif" alt="魔王邪眼小鸟游六花" width="300">
</div>

---

## 🌐 多言語自己紹介 | Multi-language Self-introduction | 多语言自我介绍

- [English](README.md)
- [中文](README.zh.md)
- [日本語](README.jp.md)
